...
read(1,*) cutoff !cutoff radius for 
...
21 continue
 ...
 do i=1,Blocks
        ...
        if (scale.eq.1) then
           scale = -2
           call beam(.false.)
        elseif (scale.eq.scales) then
           scale = -scales+1
           call beam(.false.)         ! lower imaginary scale
        else
           scale = -scale-1  ! higher imaginary scale
           call beam(.false.)
           scale = scale+2   ! lower imaginary scale
           call beam(.false.)
        endif
        write(6,*) 'real: ',NAtoms-NIdeal, ' Images: ', NIdeal
 enddo
endif
close(20)
close(1)
...
allocate(GridSize(scales))
allocate(MCELL(3,scales))
do i=1,scales
 GridSize(i)=cutoff*float(kScale)**(i-1)
 MCELL(:,i)=2.*int(BoxSize(:)/GridSize(i)/2.) !+1 ! removed the +1 since the version13 GP
 write(0,*) "MCELL: ",i, MCELL(:,i)
enddo
                                         !code does not use it, and the GP code works.
call flush(0)
 if (minval(MCELL(:,scales)).lt.2) then
  write(0,*) "ERROR: BoxSize is too small or Verlet Neighbour list Cell too large"
  call flush(0); call abort
 endif
 MCELLmax=maxval(MCELL(1,:)*MCELL(2,:)*MCELL(3,:))
 allocate(HEAD(MCELLmax,scales))
...
enddo
!else
!write(0,*) "using cuttype", cuttype, cdom
endif
...
use phase_data
implicit none
integer :: cntKept, cntIdeal, i, scl, cntReal, J, k1, k2, k3
real(4) :: tol, dist
real(4), dimension(3) :: Rij
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
 ...
 List=0
 DO I=1,NAtoms
  scl = abs(ScaleIndex(i))
  IXYZ(:)=Int( pos(:,I)*float(MCELL(:,scl)) )
  ICELL=1+IXYZ(1)+ IXYZ(2)*MCELL(1,scl)+ IXYZ(3)*MCELL(1,scl)*MCELL(2,scl)
  if(ICELL.lt.1 .or. ICELL.gt.product(MCELL(:,scl)))then
   write(0,*) "ICELL problem, particle Outside of simulation supercell", ICELL, &
              " of ( 1,",product(MCELL(:,scl)),")"
   write(0,*) "pos", pos(:,I),"particle:",I, "BoxSize:",BoxSize
   call flush(0); call abort
  endif
  List(I)=HEAD(ICELL,scl)
  HEAD(ICELL,scl)=I
 ENDDO
 write(0,*) "Neighbour List Generation complete"; call flush(0)
 !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
scl = abs(ScaleIndex(i))
tol=cutoff*kScale**(scl-1) ! scale cutoff radius is the tolerance. 
IXYZ(:)=Int( pos(:,I)*float(MCELL(:,scl)) )
outer: do k1=-1,1
   do k2=-1,1
      do k3=-1,1
         ICELL=1 + ( MOD(IXYZ(1)+k1  + MCELL(1,scl), MCELL(1,scl)) )+ &
            ( MOD(IXYZ(2)+k2  + MCELL(2,scl), MCELL(2,scl)) ) * MCELL(1,scl)+ &
            ( MOD(IXYZ(3)+k3  + MCELL(3,scl), MCELL(3,scl)) ) * MCELL(1,scl) * MCELL(2,scl)
         J=HEAD(ICELL,scl)

         DO WHILE(j .GT. 0)
            if (ScaleIndex(J).ne.scl) then
               J=List(J);cycle ! not the same scale as I and not real
            endif
            if (i.eq.j) then
               J=List(J);cycle
            endif
            Rij=(pos(:,i)-pos(:,j))*BoxSize
            dist=sqrt(sum(Rij**2))
            if (dist.lt.tol) then
               cntIdeal=cntIdeal+1
               ...
               cntKept=cntKept+1
               exit outer
            endif ! else ignore the image not within a cutoff of any real particles
            J=List(J)
         ENDDO
      enddo ! end k3
   enddo ! end k2
...
NAtoms=cntIdeal+cntReal

!Read the model from that xyz file
open(20,file='Model.MD') 
read(20,*)                
read(20,*)                
do i=1,NAtoms               
        read(20,*) ignore, pos(:,i),ScaleIndex(i),AtomTypeIndex(i)
...
write(20,*)                
do i=1,NAtoms               
    if (ScaleIndex(i).gt.0) then
        write(20,*) stype(AtomTypeIndex(i)), (pos(:,i)-0.5)*BoxSize,ScaleIndex(i),AtomTypeIndex(i)
    else
        write(20,*) " H", (pos(:,i)-0.5)*BoxSize,ScaleIndex(i),AtomTypeIndex(i)
    endif
        !pos(:,i)=pos(:,i)/BoxSize+0.5
!        if(ScaleIndex(i)<0) CntIdeal=CntIdeal+1         ! Count the numebr of ideal atoms/paticles
enddo                  
...
write(0,*), 'Totally ', cntReal,' real atoms/particles exist'
